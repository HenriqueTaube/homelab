...
TITULO_ITEM03    = '[Item 03 – Nome do produto / sistema]'
...
def add_unified_investment_table(doc, blocks, grand_total):
    # blocks: lista de (qty, valor_unit, total) — uma linha por item
...
add_body(doc, f'A presente proposta contempla o fornecimento de {TITULO_ITEM01}, {TITULO_ITEM02} e {TITULO_ITEM03}, para aplicação na operação de {CLIENTE_NOME}.')
...
# ── 5. ITEM 03 ─────────────────────────────────────────────────────────────────
doc.add_page_break()
add_heading(doc, f'5. Item 03 – {TITULO_ITEM03}')

add_heading(doc, '5.1 Quantidade', level=2)
add_scope_table(doc, ESCOPO_03)

doc.add_paragraph().paragraph_format.space_after = Pt(6)
add_heading(doc, '5.2 Dados de Aplicação', level=2)
add_kv_table(doc, [
    ('[Campo]', '[Valor]'),
    ('[Campo]', '[Valor]'),
    ('[Campo]', '[Valor]'),
...
doc.add_paragraph().paragraph_format.space_after = Pt(6)
add_heading(doc, '5.3 Características Construtivas', level=2)
add_kv_table(doc, [
    ('[Componente]', '[Descrição construtiva]'),
    ('[Componente]', '[Descrição construtiva]'),
    ('[Componente]', '[Descrição construtiva]'),
...
doc.add_paragraph().paragraph_format.space_after = Pt(6)
add_heading(doc, '5.4 Recursos Operacionais', level=2)
for item in ['[Recurso operacional 01]', '[Recurso operacional 02]', '[Recurso operacional 03]']:
    add_body(doc, item, bullet=True)

doc.add_paragraph().paragraph_format.space_after = Pt(6)
add_heading(doc, '5.5 Benefícios Operacionais', level=2)
for item in ['[Benefício operacional 01]', '[Benefício operacional 02]', '[Benefício operacional 03]']:
    add_body(doc, item, bullet=True)
doc.add_paragraph()
...
# ── 6. INVESTIMENTO ────────────────────────────────────────────────────────────
doc.add_page_break()
add_heading(doc, '6. Investimento')

add_unified_investment_table(doc, [
    (TITULO_ITEM01, *INV_01),
    (TITULO_ITEM02, *INV_02),
    (TITULO_ITEM03, *INV_03),
], GRAND_TOTAL)
doc.add_paragraph()

# ── 7. PRAZO DE ENTREGA ────────────────────────────────────────────────────────
add_heading(doc, '7. Prazo de Entrega')
...
add_body(doc, 'Item 03:  [XX] dias corridos após confirmação do pedido.')

add_heading(doc, '8. Condições de Pagamento')
...
for item in ['[X]% no pedido', '[X]% na entrega', 'Saldo em [XX/XX] DDL']:
    add_body(doc, item, bullet=True)
add_body(doc, 'Item 03:', bold=True)
for item in ['[X]% no pedido', '[X]% na entrega', 'Saldo em [XX/XX] DDL']:
    add_body(doc, item, bullet=True)
...
add_heading(doc, '9. Dados Complementares')
add_kv_table(doc, [
    ('Frete',                   'FOB'),
...
add_heading(doc, '11. Validade da Proposta')
...
out = f'/home/henrique/projetos/editor/proposta/models/MODELO 3 ITENS.docx'
doc.save(out)
